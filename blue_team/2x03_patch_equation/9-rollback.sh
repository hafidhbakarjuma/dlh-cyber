...
if pkg not in packages_dict:
    # Fallback search in packages list if structure varies
    target_version = None
...
# Get current installed version before rollback
...
# Execute rollback via apt-get install --allow-downgrades
print(f"[*] Downgrading {pkg}...")
env = os.environ.copy()
env["DEBIAN_FRONTEND"] = "noninteractive"
downgrade_cmd = ["apt-get", "install", "-y", "--allow-downgrades", f"{pkg}={target_version}"]
res = subprocess.run(downgrade_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, env=env)
...
if not affected_services:
    # Default fallback if package wasn't explicitly linked in map
