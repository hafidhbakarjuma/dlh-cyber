...
# 1. Disable root login - addresses Crimson Tide Phase 3 (SSH lateral movement)
set_sshd_param "PermitRootLogin" "no"

# 2. Disable password authentication - addresses Finding 009 (SSH password auth)
...
# 5. Limit authentication attempts - addresses brute-force attacks
set_sshd_param "MaxAuthTries" "3"

# 6. Client idle timeout interval (300 seconds = 5 minutes)
set_sshd_param "ClientAliveInterval" "300"

# 7. Client alive count max (2 counts -> 10 minute total idle timeout)
set_sshd_param "ClientAliveCountMax" "2"

# 8. Restrict allowed users - addresses MedDefense access control policy
set_sshd_param "AllowUsers" "medadmin sysadmin"

# 9. Enforce Protocol 2
