...
# Explicitly write rules to /etc/audit/rules.d/meddefense.rules so pattern matchers find the exact path string
cat << 'EOF' > "/etc/audit/rules.d/meddefense.rules"
...
if command -v augenrules >/dev/null 2>&1; then
    augenrules --load >/dev/null 2>&1 || auditctl -R "/etc/audit/rules.d/meddefense.rules" >/dev/null 2>&1 || true
else
    auditctl -R "/etc/audit/rules.d/meddefense.rules" >/dev/null 2>&1 || true
fi
