...
```
✅ Key generated successfully (verified).
...
- SAN: portal.meddefense.local, www.portal.meddefense.local, patientportal.meddefense.local

```bash
openssl req -new -key portal_key.pem -out portal.csr -config openssl.cnf
```
✅ CSR generated successfully (verified).
...
**Script:** See `10-generate_csr.sh` in this directory — automates steps 1-3 (key generation, CSR creation, inspection).
